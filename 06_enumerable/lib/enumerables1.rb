...
def array_sum_with_index(arr)
	...
	arr.map.with_index {|x, i| x * i }.reduce(:+)
end
...
def price_is_right(bids, actual_retail_price)
	bids.select {|x| x <= actual_retail_price}.max
end
...
def num_factors(num)
	(1..num).select {|x| num % x == 0 }.length
end

def at_least_n_factors(numbers, n)
	numbers.select {|x| num_factors(x) >= n}
end
...
def ordered_vowel_word?(word)
	arr = word.chars.select {|x| "aeiou".include?(x)}
	arr == arr.sort
end

def ordered_vowel_words(words)
	words.select { |x| ordered_vowel_word?(x) }
end
